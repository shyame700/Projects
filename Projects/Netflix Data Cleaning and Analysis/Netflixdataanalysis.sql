...
,rating,case when duration is null then rating else duration end as duration, description into netflix
from cte
select * from netflix_raw
...
--missing values are filling by same director values
select *
from netflix_raw 
...
--inserting the table into netfilx_country table
...
------ duration : where ever duration is null populate with rating
...
--inserting the data into new final table netflix
with cte as (
select *
,row_number() over(partition by title,type order by show_id) as rn
from netflix_raw
...
--5. find the list of directors  who have created  horror and comedy moveis both 
--display director names  along with number of comedy and horror movies directed by them

select nd.director
,COUNT(distinct case when ng.genre='Comedies' then n.show_id end) as no_comedy
,COUNT(distinct case when ng.genre='Horror Movies' then n.show_id end) as no_of_horror
from netflix n
inner join netflix_genre ng on n.show_id=ng.show_id
inner join netflix_directors nd on n.show_id=ng.show_id
